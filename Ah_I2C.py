...
class Ah_I2C(object):
  ...
  def __init__(self, address, busnum=-1, debug=False):
    ...
    self.bus = smbus.SMBus(busnum if busnum >= 0 else Ah_I2C.getPiI2CBusNumber())
    self.debug = debug
  ...
  def readU16(self, reg, little_endian=True):
    ...
    try:
      ...
      print ("Error reading address: 0x%02X, reg: 0x%02X-%s" % (self.address, reg, self.reg_list[reg] ))
      return (0,True)
