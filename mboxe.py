import struct
import time
from Ah_I2C import Ah_I2C

class Mboxe():
    """Classe définissant le paramétrage et commande d'une M-BOXE
    voir doucment de Specification Fonctionnelle"""
    ...
    def __init__(self,adresse):
        ...
        self.skeletonPosition_ref={"read":0x2D, "write":0x3D, "type":bytearray(), "size":4}
        ...
        self.limitBW_ref={"read":0x20, "write":0x30, "type":int(), "size":2, "min":350, "max":750}
        self.limitBW = self.limitBW_ref["min"]
        self.limitFW_ref={"read":0x21, "write":0x31, "type":int(), "size":2, "min":750, "max":1150}
        ...
        self.positionCurrent_ref={"read":0x40, "write":0x50, "type":int(), "size":2, "min":self.limitBW, "max":self.limitFW}
        self.positionCurrent = 750
    ...
    def get_state(self):
        reg= self.state_ref["read"]
        (self.state, error) = self.get_UChar(reg)
        if self.debug & error:
            print ("Error reading address: %d, reg: 0x%02X-%s" % (self.id, reg, self.reg_list[reg]) )
    ...
    def get_currentCurrent(self):
        ...
        if current>0: self.currentCurrent= current/10
        else: self.currentCurrent= 0
        if self.debug & error:
            print ("Error reading address: %d, reg: 0x%02X-%s" % (self.id, reg, self.reg_list[reg]) )
